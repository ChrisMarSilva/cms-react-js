...
const UserTable = () => {
    ...
    const deleteUser = (id) => {
        users.set($users.map(u => { if (u._id !== id) { return u } }))
    }
            ...
            <tbody className="bg-white dark:bg-slate-800">
                {
                    $users.map(u => {
                        return u && <tr key={u._id}>
                            ...
                            <td className="border-b border-slate-100 dark:border-slate-700 p-4 pr-8 text-slate-500 dark:text-slate-400"> <button onClick={() => setUser(u)} className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded"> Edit </button> </td>
                            <td className="border-b border-slate-100 dark:border-slate-700 p-4 pr-8 text-slate-500 dark:text-slate-400"> <button onClick={() => deleteUser(u._id)} className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded"> Delete </button> </td>
                        </tr>
                    })
                }
            </tbody>
