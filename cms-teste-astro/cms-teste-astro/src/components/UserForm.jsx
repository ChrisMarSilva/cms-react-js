...
const UserForm = () => {
    ...
    const createOrUpdateUser = (e) => {
        e.preventDefault()
        if ($user._id !== "") {
            users.set($users.map(u => { if (u._id === $user._id) { return $user } else { return u } }))
        } else {
            users.set([...$users, { ...$user, _id: nanoid(8) }])
        }
        console.log($users);
        user.set({ _id: "", name: "", email: "", password: "" })
    }
    ...
    return <>
        ...
        <div className="flex justify-center">
            <form onSubmit={createOrUpdateUser} className="w-full max-w-lg">
                ...
                <button type="submit" className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded"> Button </button>
            </form>
        </div>
    </>
}
