...
export default function CadBdClienteModalAlt(props: any) {

    useEffect(() => {
        // console.log('CadBdClienteModalAlt.Entrar')
        //props.handleLimpar();
        props.mutationListarIF.mutate();

        return () => {
            // console.log('CadBdClienteModalAlt.Sair')
            props.handleLimpar()
        }
    }, [])
        ...
        <>
            ...
            <Modal {...props} show={props.show} onHide={props.handleHideModal} dialogClassName="modal-dialog modal-dialog-centered modal-lg" contentClassName="modal-content" size="lg" backdrop="static" keyboard={false}>
                <Modal.Header closeButton className="modal-header">
                    <Modal.Title>Alterar Banco de Dados</Modal.Title>
                </Modal.Header>
                <Modal.Body className="modal-body p-0">
                    <form className="body-pad row w-100 border-bottom-dashed-danger" onSubmit={props.handleSubmit}>
                        ...
                        <div className="col-6">
                            <div className="mb-3">
                                <label htmlFor="txtStBd" className="form-label">Situação*</label>
                                <select value={props.txtStBd} onChange={props.handleInput} className="form-select" id="txtStBd" name="txtStBd" required>
                                    <option value=""></option>
